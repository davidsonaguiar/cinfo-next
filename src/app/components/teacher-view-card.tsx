...
export default function TeachereViewCard(props: Props) {
    return (
      <div className="w-fit sm:w-full p-2 sm:p-3 bg-white border border-border grid grid-cols-1 sm:grid-cols-[200px,auto] gap-3 relative">
        <img
          src={"/images/teacher/" + props.avatar}
          width={200}
          height={230}
          className="w-[200px] h-[230px] mx-auto sm:mx-0 object-cover"
        />
        <div className="w-full">
          <h3 className="mb-1 sm:text-2xl font-semibold text-lg text-center sm:text-left">{props.name}</h3>
          <p className="font-semibold text-center text-sm sm:text-base sm:text-left">
            <span>E-mail: </span>
            <a href={`mailto:${props.email}`} className="text-sky-600 font-normal">
              {props.email}
            </a>
          </p>
